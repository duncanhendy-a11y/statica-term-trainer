...
export default function StudyPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  }, [decoded, shuffle, diff]);

  const filtered = diff === "all" ? cards : cards.filter((c) => c.difficulty === diff);
  const display  = shuffle ? [...filtered].sort(() => Math.random() - 0.5) : filtered;
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          ...
          <div className="flex gap-2 items-center">
            ...
            <button onClick={() => setShuffle((s) => !s)}
              className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${shuffle ? "bg-[#F36E22]/20 border-[#F36E22] text-[#F36E22]" : "bg-[#2E2E2E] border-[#3A3A3A] text-[#8A8A8A]"}`}>
              🔀 Shuffle
            </button>
            <Link href={`/quiz/${encodeURIComponent(category)}`}
              className="text-xs px-3 py-1.5 bg-[#F36E22]/10 hover:bg-[#F36E22]/20 border border-[#F36E22]/30 text-[#F36E22] rounded-lg transition-all">
              Switch to Quiz →
            </Link>
          </div>
        </div>
